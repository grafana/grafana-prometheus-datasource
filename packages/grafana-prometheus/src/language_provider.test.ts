...
describe('PrometheusLanguageProvider', () => {
  ...
  describe('constructor', () => {
    ...
    it('should stop exposing an unavailable SearchApiClient', () => {
      const datasourceWithSearchAPI = {
        ...defaultDatasource,
        hasSearchApiSupport: () => true,
      } as unknown as PrometheusDatasource;
      const provider = new PrometheusLanguageProvider(datasourceWithSearchAPI);
      const searchClient = provider.getSearchApiClient();

      jest.spyOn(searchClient!, 'isAvailable').mockReturnValue(false);

      expect(provider.getSearchApiClient()).toBeUndefined();
      expect(provider.hasSearchSupport()).toBe(false);
    });
  });
