...
// OAuth2ClientCredentialsConfig holds the OAuth2 client-credentials grant settings read
// from the datasource configuration. ClientSecret comes from
// DecryptedSecureJSONData["oauth2ClientSecret"], the rest from jsonData.
...
// OAuth2ClientCredentials returns a middleware that, when enabled in the datasource
// configuration, fetches and caches an OAuth2 access token via the client-credentials
// grant and attaches it as an Authorization: Bearer header to outgoing Prometheus
// requests. Token fetch requests are made through "next" so they honor the datasource's
// own TLS/proxy configuration.
//
...
func OAuth2ClientCredentials(logger log.Logger, oauth2Cfg OAuth2ClientCredentialsConfig) sdkhttpclient.Middleware {
