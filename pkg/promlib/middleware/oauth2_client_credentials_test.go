...
func TestOAuth2ClientCredentialsMiddleware(t *testing.T) {
	...
	t.Run("Name should be correct", func(t *testing.T) {
		mw := OAuth2ClientCredentials(logger, OAuth2ClientCredentialsConfig{Enabled: true, ClientSecret: "secret"})
		middlewareName, ok := mw.(httpclient.MiddlewareName)
		require.True(t, ok)
		require.Equal(t, oauth2ClientCredentialsMiddlewareName, middlewareName.MiddlewareName())
	})

	t.Run("With a zero config should not apply middleware", func(t *testing.T) {
		finalRoundTripper := httpclient.RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
			require.Empty(t, req.Header.Get("Authorization"))
			return &http.Response{StatusCode: http.StatusOK, Body: http.NoBody}, nil
		})
		mw := OAuth2ClientCredentials(logger, OAuth2ClientCredentialsConfig{})
	...
	t.Run("With Enabled=false should not apply middleware", func(t *testing.T) {
		finalRoundTripper := httpclient.RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
			require.Empty(t, req.Header.Get("Authorization"))
			return &http.Response{StatusCode: http.StatusOK, Body: http.NoBody}, nil
		})
		mw := OAuth2ClientCredentials(logger, OAuth2ClientCredentialsConfig{Enabled: false, ClientSecret: "secret"})
		rt := mw.CreateMiddleware(httpclient.Options{}, finalRoundTripper)
		require.NotNil(t, rt)
