...
// LenientStringSlice ignores a value it cannot read, so a scopes list stored as something
// other than an array of strings does not fail the datasource.
...
func (s *LenientStringSlice) UnmarshalJSON(data []byte) error {
	...
	dropped("stringSlice", "unknown", data)
	return nil
}
